 ...
 */
 ...
 export default (navermaps)=>{
    ...
    var MarkerClustering = function(options) {
        ...
        this.setOptions(navermaps.Util.extend({}, this.DEFAULT_OPTIONS, options), true);
        this.setMap(options.map || null);
    };

    navermaps.Util.ClassExtend(MarkerClustering, navermaps.OverlayView, {
        onAdd: function() {
            var map = this.getMap();

            this._mapRelations = navermaps.Event.addListener(map, 'idle', navermaps.Util.bind(this._onIdle, this));
        ...
        draw: navermaps.Util.noop,

        onRemove: function() {
            navermaps.Event.removeListener(this._mapRelation);
        ...
        setOptions: function(newOptions) {
            ...
            if (typeof newOptions === 'string') {
                ...
                navermaps.Util.forEach(newOptions, function(value, key) {
                    if (key !== 'map') {
                        _this.set(key, value);
                    }
                });
        ...
        getOptions: function(key) {
            ...
            if (key !== undefined) {
                ...
                navermaps.Util.forEach(_this.DEFAULT_OPTIONS, function(value, key) {
        ...
        /**
         * 클러스터로 구성할 마커를 반환합니다.
         * @return {Array.<navermaps.Marker>} 클러스터로 구성할 마커
         */
        getMarkers: function() {
        ...
        /**
         * 클러스터로 구성할 마커를 설정합니다.
         * @param {Array.<navermaps.Marker>} markers 클러스터로 구성할 마커
         */
        ...
        /**
         * 클러스터 마커 아이콘을 반환합니다.
         * @return {Array.<navermaps.Marker~ImageIcon | navermaps.Marker~SymbolIcon | navermaps.Marker~HtmlIcon>} 클러스터 마커 아이콘
         */
        getIcons: function() {
        ...
        /**
         * 클러스터 마커 아이콘을 설정합니다.
         * @param {Array.<navermaps.Marker~ImageIcon | navermaps.Marker~SymbolIcon | navermaps.Marker~HtmlIcon>} icons 클러스터 마커 아이콘
         */
        ...
        _createClusters: function() {
            ...
            for (var i = 0, ii = markers.length; i < ii; i++) {
                ...
                this._markerRelations.push(navermaps.Event.addListener(marker, 'dragend', navermaps.Util.bind(this._onDragEnd, this)));
            }
        },
        ...
        _clearClusters: function() {
            ...
            navermaps.Event.removeListener(this._markerRelations);
         ...
         * @param {navermaps.LatLng} position 위/경도
         * @return {Cluster} 클러스터
         */
    ...
    Cluster.prototype = {
        ...
        /**
         * 클러스터에 마커를 추가합니다.
         * @param {navermaps.Marker} marker 클러스터에 추가할 마커
         */
         ...
         */
        destroy: function() {
            navermaps.Event.removeListener(this._relation);
        ...
        /**
         * 클러스터 중심점을 반환합니다.
         * @return {navermaps.LatLng} 클러스터 중심점
         */
        getCenter: function() {
        ...
        /**
         * 클러스터 경계 영역을 반환합니다.
         * @return {navermaps.LatLngBounds} 클러스터 경계 영역
         */
        getBounds: function() {
      ...
      /**
         ...
         * @return {navermaps.Marker[]} 클러스터를 구성하는 마커 객체 집합
         */
      getClusterMember: function() {
         ...
         * @param {navermaps.LatLng} latlng 위/경도
        ...
        enableClickZoom: function() {
            ...
            this._relation = navermaps.Event.addListener(this._clusterMarker, 'click', navermaps.Util.bind(function(e) {
        ...
        _calcBounds: function(position) {
            var map = this._markerClusterer.getMap(),
                bounds = new navermaps.LatLngBounds(position.clone(), position.clone()),
            ...
            var max_px_x = Math.min(map_max_px.x, max_px.x),
                ...
                newMax = proj.fromOffsetToCoord(new navermaps.Point(max_px_x, max_px_y)),
                newMin = proj.fromOffsetToCoord(new navermaps.Point(min_px_x, min_px_y));

            return new navermaps.LatLngBounds(newMin, newMax);
        },
        ...
        _getIndex: function(count) {
            ...
            if (navermaps.Util.isFunction(indexGenerator)) {
                return indexGenerator(count);
            } else if (navermaps.Util.isArray(indexGenerator)) {
                var index = 0;
         ...
         * @param {navermaps.Marker} marker 마커
         ...
         * @param {Array.<navermaps.Marker>} markers 마커 배열
         * @return {navermaps.Point} 마커들의 중심 좌표
         * @private
         */
        _calcAverageCenter: function(markers) {
            ...
            return new navermaps.Point(averageCenter[0], averageCenter[1]);
        }
